...
@section('content')
    ...
    <!-- Modal edit Point-->
    <div class="modal fade" id="editpointModal" tabindex="-1" aria-labelledby="createpointModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('points.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="editpointModalLabel">Usulan Titik Pembangkit Listrik</h5>
        ...
        var point = L.geoJson(null, {
            onEachFeature: function(feature, layer) {
                ...
                layer.on({
                    click: function(e) {
                        // menampilkan data ke dalam modal
                        $('textarea[name="nama_lokasi"]').val(properties.nama_lokasi);
                        $('input[name="tenaga_pembangkit"]').val(properties.tenaga_pembangkit);
                        $('textarea[name="wilayah"]').val(properties.wilayah);
                        $('textarea[name="alasan"]').val(properties.alasan);
                        $('input[name="surveyor"]').val(properties.surveyor);
                        $('#geom_points').val(objectGeometry);
